...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($user = Auth::attempt($params, true)) {
			if ($checkExistEmail && !$checkExistEmail->kscinus) {
				$settings = Helper::getSetting();

				// $c = new SDK();
				// $player = $c->PlayerCreate([
				// 	'email'	=> $checkExistEmail->email,
				// 	'username'	=> $checkExistEmail->name,
				// 	'api-key'	=> $settings['kscinus_pub_key']
				// ], $settings['kscinus_pvt_key']);

				// if ($player) {
				// 	if (isset($player['id'])) {
				// 		$checkExistEmail->kscinus = TRUE;
				// 		$checkExistEmail->save();
				// 	}
				// }
			}
